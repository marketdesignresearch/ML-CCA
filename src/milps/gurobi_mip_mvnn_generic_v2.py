...
class GUROBI_MIP_MVNN_SINGLE_BIDDER_UTIL_MAX:
    ...
    def generate_mip(self,
        ...
        for j in range(len(self.capacities)):
            self.y_variables.append(self.mip.addVars([i for i in range(len(self.capacities[j]))], name=f"x_{j}_", vtype = GRB.BINARY))  # the "input variables, i.e. the first y level"
